...
Synchronizes the vector index with your S3 bucket, detecting new, modified, or deleted files.
...
:::caution[Important]
The `force` parameter should **ONLY** be set to `true` when explicitly needed (e.g., "force reindex", "rebuild everything from scratch"). 

**Full reindex is expensive:**
- Re-downloads all files from S3
- Regenerates all embeddings (calls to Ollama/OpenAI)
...
Only processes new, modified, or deleted files based on ETag comparison. This is fast and efficient for regular updates.
...
Rebuilds the entire index from scratch. Use ONLY when explicitly needed (see "When to Use" below).
...
#### ✅ Incremental Sync (`force: false` or omit parameter) - RECOMMENDED

**Always use for:**
- 📝 Documentation has been updated in S3
- ➕ Adding newly uploaded files
...
**Performance:**
- ⚡ Fast (seconds to minutes depending on changes)
- 💰 Cost-effective (only processes changes)
- 📊 Efficient (minimal API calls)

#### ⚠️ Full Reindex (`force: true`) - USE SPARINGLY

**Only use when explicitly needed:**
- 🔄 Switching embedding providers (Ollama ↔ OpenAI)
- ⚙️ Changing chunk size or overlap settings
- 🔧 Suspecting index corruption
- 🎯 After significant RAG configuration changes
- 🆘 Troubleshooting persistent issues

**❌ DO NOT use for:**
- Regular documentation updates (use incremental instead)
- Routine maintenance
- Adding a few new files
- "Just to be safe" reasoning

**Performance Impact:**
- 🐌 Slow (minutes to hours depending on corpus size)
- 💸 Expensive (all files reprocessed, all embeddings regenerated)
- 📈 High API usage (S3 downloads + embedding calls)
